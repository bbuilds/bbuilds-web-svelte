...
describe('Eyebrow', () => {
	...
	it('applies text-muted class for light tone (default)', () => {
		const { container } = render(Eyebrow, { text: 'label' });
		const div = container.querySelector('div');
		expect(div?.className).toContain('text-muted');
		expect(div?.className).not.toContain('text-muted-dark');
	});

	it('applies text-muted-dark class for dark tone', () => {
		const { container } = render(Eyebrow, { text: 'label', tone: 'dark' });
		const div = container.querySelector('div');
		expect(div?.className).toContain('text-muted-dark');
	});

	it('forwards extra class to the wrapper div', () => {
		const { container } = render(Eyebrow, { text: 'label', class: 'text-sm mb-4' });
		const div = container.querySelector('div');
		expect(div?.className).toContain('text-sm');
		expect(div?.className).toContain('mb-4');
	});

	it('renders nothing when neither text nor children are provided', () => {
