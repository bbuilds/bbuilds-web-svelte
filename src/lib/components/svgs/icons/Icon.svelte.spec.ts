...
describe('Icon', () => {
	...
	it('uses the default viewBox of 0 0 24 24', () => {
		const { container } = render(Icon, { children: pathSnippet });
		expect(container.querySelector('svg')?.getAttribute('viewBox')).toBe('0 0 24 24');
	});

	it('applies a custom viewBox when provided', () => {
		const { container } = render(Icon, { children: pathSnippet, viewBox: '0 0 20 20' });
		expect(container.querySelector('svg')?.getAttribute('viewBox')).toBe('0 0 20 20');
	});

	it('applies class prop to the SVG', () => {
		const { container } = render(Icon, { children: pathSnippet, class: 'h-5 w-5' });
